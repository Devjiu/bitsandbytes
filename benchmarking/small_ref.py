...
def dequant_nf4_fp16(
    ...
    print(" shapes: ", A_nf4.shape, out_B.shape, quant_state_code.shape, absmax.shape)
    print(" grid: ", grid)
    print("absmax: ", absmax)
    print("-----------\n")
    dequant_kernel[grid](
...
if triton.runtime.driver.active.get_current_target().backend != "xpu":
    raise RuntimeError("Device is not xpu: ", triton.runtime.driver.active.get_current_target())
else:
...
def mm4_ref(batch=1, seq=1, model=2, hidden=128):
    ...
    B_1 = torch.HalfTensor(
        [
            [-0.01041, 0.14868, 0.05603, 0.19592],
    ...
    B_dq_1 = dequantize_nf4(B_nf4_1, B_dq_1, quant_state_code, absmax, blocksize)
...
def mm4(batch=1, seq=1, model=1, hidden=736):
    ...
    if blocksize != state_nf4.blocksize:
        raise RuntimeError("Mismatch of quantization blocksizes: ", blocksize, " != ", state_nf4.blocksize)
    absmax = get_absmax(B, blocksize)
    quant_state_code = torch.HalfTensor(quant_state_code)
